...
export const CameraTool = () => {
  ...
  const renderButtons = () => {
    if (img)
      return (
        <>
          <button className={CameraToolCss.againPicBtn} onClick={retry}>Volver a capturar foto</button>
          <button className={CameraToolCss.sendPicBtn} onClick={sendImg}>Enviar foto</button>
        </>
      )
    else
        return (
  ...
  if (!data && !isLoading && !error)
  {
    return (
      <div>
        ...
        <input type="file" accept="image/*" onChange={getImgFromPc} className={CameraToolCss.uploadBtn}></input>
        {renderButtons()}
